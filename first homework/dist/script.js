...
/* harmony import */ var _modules_showImage__WEBPACK_IMPORTED_MODULE_6__ = __webpack_require__(/*! ./modules/showImage */ "./src/js/modules/showImage.js");
...
window.addEventListener('DOMContentLoaded', function () {
  ...
  var modalState = {}; // Передаём пустой объект куда будем вносить данные с формы-калькулятора

  var deadLine = '2021-01-01'; // дэд лайн до которого будет работать таймер

  Object(_modules_changeModalState__WEBPACK_IMPORTED_MODULE_4__["default"])(modalState); // Передаём пустой объект в который будут записываться значения с формы-калькулятора

  Object(_modules_modals__WEBPACK_IMPORTED_MODULE_1__["default"])();
  Object(_modules_tabs__WEBPACK_IMPORTED_MODULE_2__["default"])('.glazing_slider ', '.glazing_block', '.glazing_content', 'active'); // Табы с остиклением

  Object(_modules_tabs__WEBPACK_IMPORTED_MODULE_2__["default"])('.decoration_slider', '.no_click', '.decoration_content > div > div', 'after_click'); // Для табов с видом отделки

  Object(_modules_tabs__WEBPACK_IMPORTED_MODULE_2__["default"])('.balcon_icons', '.balcon_icons_img', '.big_img > img', 'do_image_more', 'inline-block');
  Object(_modules_forms__WEBPACK_IMPORTED_MODULE_3__["default"])(modalState); // Передаём объект в котором храняться значения с формы-калькулятора 

  Object(_modules_timer__WEBPACK_IMPORTED_MODULE_5__["default"])('.container1', deadLine); // Таймер 

  Object(_modules_showImage__WEBPACK_IMPORTED_MODULE_6__["default"])(); // Показ картинок в увеличенном формате
});
...
var changeModalState = function changeModalState(state) {
  ...
  Object(_checkNumInputs__WEBPACK_IMPORTED_MODULE_1__["default"])('#width'); // Вызываем ф-ию с валидацией для инпутов с расчётом ширины и высоты окон

  Object(_checkNumInputs__WEBPACK_IMPORTED_MODULE_1__["default"])('#height');

  function bindActionToElems(event, elem, prop) {
    // Что делаем с элементами? 1. Событие обработчика, 2. конкретный элемент, 3. Значение элемента, т.е когда будем записывать значение в объект formDate, это значение будет выступать ключём
    elem.forEach(function (item, i) {
      item.addEventListener(event, function () {
        switch (item.nodeName) {
          // Если элемент span, это для формы окна, то записывем в объект с ключём prop позицию в массиве формы окна их там несколько
          case 'SPAN':
            state[prop] = i;
            break;

          case 'INPUT':
            // Если инпут, и тип чекбокс и позиция = 0, то значение будет равно "Холодное" иначе "Теплое"
            if (item.getAttribute('type') === 'checkbox') {
              i === 0 ? state[prop] = "Холодное" : state[prop] = "Теплое";
              elem.forEach(function (box, j) {
                // Со всех чекбоксов снимаем галочку, если выбранный эл-т = перебираемому, значит ставим галочку
                box.checked = false;
              ...
              // Если не чекбокс, то просто считываем значение
          ...
          case 'SELECT':
            // Также считываем просто значение
            state[prop] = item.value;
            break;
        }
...
/* harmony import */ var _checkNumInputs__WEBPACK_IMPORTED_MODULE_5__ = __webpack_require__(/*! ./checkNumInputs */ "./src/js/modules/checkNumInputs.js");
 ...
 // Добавляем ф-ию с валидацией цифр

var forms = function forms(state) {
  // Получем объект переданный из main.js, если таков имеется
  var form = document.querySelectorAll('form'),
      inputs = document.querySelectorAll('input');
  Object(_checkNumInputs__WEBPACK_IMPORTED_MODULE_5__["default"])('input[name="user_phone"]'); // Валидация цифр только для формы с атрибтом name="user_phone"
    ...
    return regeneratorRuntime.async(function postData$(_context) {
      while (1) {
        switch (_context.prev = _context.next) {
          case 0:
            // Отправляем данные на сервер, только после этого выдаем результат ввиде одного из сообщений из объекта message
  ...
  form.forEach(function (item) {
    ...
    item.addEventListener('submit', function (e) {
      e.preventDefault();
      var statusMessage = document.createElement('div'); // Создаем блок где будет храниться статус работы формы

      statusMessage.classList.add('status');
      item.appendChild(statusMessage);
      var formData = new FormData(item); // Создаем форм дату

      if (item.getAttribute('data-calc') === "end") {
        // В него помещаем данные переданные из объекта, если форма калькулятор
      ...
      .then(function (res) {
        // Получаем промис, который будет выдавать результат
        console.log(res);
...
var modals = function modals() {
  function bindModal(triggerSelector, modalSelector, closeSelector) {
    ...
    // 1. Получем объект с которым совершили действие, 2. какое окно будем открывать, 3. селектор, который закрывает окно, 4. При нажатии на подложку можно закрыть элемент?
    var trigger = document.querySelectorAll(triggerSelector),
        modal = document.querySelector(modalSelector),
        close = document.querySelector(closeSelector),
        windows = document.querySelectorAll('[data-modal]'),
        // Все модальные окна по с атрибутом [data-modal]
    scroll = calcScroll();
    trigger.forEach(function (item) {
      // Перебераем все окна и им навешиваем обработчик события
    ...
    modal.addEventListener('click', function (e) {
      if (e.target === modal && closeClickOverlay) {
        // Если нажимаем на подложку, значит можно закрыть эл-т
        windows.forEach(function (item) {
          item.style.display = 'none';
        });
  ...
  function showModalByTime(selector, time) {
    // Показываем модальное окно через несколько секунд
    setTimeout(function () {
  ...
  function calcScroll() {
    // Убираем дергание страницы, просто добавляем маргин body равный ширине скролла
...
var showImage = function showImage() {
  // Создаем див, каринку, а также область с классом 'works' помещаем в переменную
  ...
  workSection.addEventListener('click', function (e) {
    // Для рабочей области добавляем событие
    e.preventDefault();
    var target = e.target;

    if (target && target.classList.contains('preview')) {
      // Если клик совершен был по картинке, то показываем конкретную картинку с увеличением
    ...
    if (target && target.matches('div.popup')) {
      // Если клик по подложке, т.е не по самой картинке, то скрываем эл-т
      imgPopup.style.display = 'none';
      document.body.style.overflow = '';
    }
  });
...
var tabs = function tabs(headerSelector, tabSelector, contentSelector, activeClass) {
  ...
  // 1. Область в которой находяться табы, 2. все табы, 3. контент под табами, 4. класс активности который будем добавлять при нажатии, 5. Для разных эл-тов разные отображения, по дефолту блок, можно грид и флекс...
  var header = document.querySelector(headerSelector),
      tab = document.querySelectorAll(tabSelector),
      content = document.querySelectorAll(contentSelector);

  function hideTabContent() {
    // Скрываем весь контент под табами
    content.forEach(function (item) {
      item.style.display = 'none';
    });
    tab.forEach(function (item) {
      // Удаляем все классы, которые задают переход
  ...
  function showTabContent() {
    ...
    // По дефолту показываем перывый таб и его контент
  ...
  header.addEventListener('click', function (e) {
    // Делегирование на табы в шапке
    var target = e.target;

    if (target && (target.classList.contains(tabSelector.replace(/\./, "")) || // Убираем точку, чтобы все было окей
    target.parentNode.classList.contains(tabSelector.replace(/\./, "")))) {
      tab.forEach(function (item, i) {
        // Перебираем эл-ты показываем лишь тот, который нам надо
...
var timer = function timer(id, deadline) {
  // Блок в который передадуться значения таймера
  var addZero = function addZero(num) {
    // Если значение меньше 10 то добавить нуль в начало
    if (num <= 9) {
      return '0' + num;
    } else {
      return num;
  ...
  var getNewTime = function getNewTime(deadline) {
    // Получаем дедлайн
    var t = Date.parse(deadline) - Date.parse(new Date()),
        // Получем разницу во времени
    seconds = Math.floor(t / 1000 % 60),
        // Получаем значения сек, мин, часов и дней
  ...
  var setClock = function setClock(selector, deadline) {
    // Устанавливаем время в блок, который мы хотим
    ...
    function updateClock() {
      var t = getNewTime(deadline); // Получаем объект с данными
      ...
      if (t.total <= 0) {
        // Если разница меньше или равна 0 то прекратить отсчет
        days.textContent = '00';
