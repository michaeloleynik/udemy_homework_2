...
window.addEventListener('DOMContentLoaded', function () {
  ...
  Object(_modules_burger__WEBPACK_IMPORTED_MODULE_9__["default"])('.burger-menu', '.burger'); //accordion('.accordion-heading', '.accordion-block');
...
var drop = function drop() {
  ...
  var fileInputs = document.querySelectorAll('[name="upload"]'); // все импуты с именем загрузка

  ['dragenter', 'dragleave', 'dragover', 'drop'].forEach(function (eventName) {
    // Навешиваем 4 события на все кнопки
  ...
  ['dragenter', 'dragover'].forEach(function (eventName) {
    // Для событий над рабочей областью навешиваем доп ф-ию ввиде добавления рамки и фона
    fileInputs.forEach(function (input) {
  ...
  ['dragleave', 'drop'].forEach(function (eventName) {
    // Для событий вне рабочей области навешиваем доп ф-ию ввиде удаления рамки и фона
    fileInputs.forEach(function (input) {
      input.addEventListener(eventName, function () {
  ...
  fileInputs.forEach(function (input) {
    // при событии сброса файла 
    input.addEventListener('drop', function (e) {
      input.files = e.dataTransfer.files; // Получаем наз-е файла

      var dots;
      var arr = input.files[0].name.split('.'); // Разбиваем название в массив, деля наз-е и расширение

      arr[0].length > 6 ? dots = "..." : dots = '.'; // Если длина названия > 6 символов, значит в переменную dots 3 точки, иначе 1

      var name = arr[0].substring(0, 6) + dots + arr[1]; // Получаем из названия первые 6 символов, добавляем точку/точки и расширение файла
...
var mask = function mask(selector) {
  var setCursorAt = function setCursorAt(pos, elem) {
    elem.focus();
  ...
  function createMask(e) {
    ...
    this.value = matrix.replace(/./gi, function (a) {
...
var modals = function modals() {
  ...
  var isPressed = function isPressed(selector) {
...
var scrolling = function scrolling(upSelector) {
  ...
  links.forEach(function (link) {
    // Навешиваем обработчик на ссылки, кнопки
    link.addEventListener('click', function (event) {
      event.preventDefault();
      var widthTop = document.documentElement.scrollTop,
          // Сколько отматали от начала
      hash = this.hash,
          // Хеш ссылки (#up например)
      toBlock = document.querySelector(hash).getBoundingClientRect().top,
          // Высота от начала в конкретного блока
      start = null; // Начальная позиция

      requestAnimationFrame(step);

      function step(time) {
        // time передаётся автоматически сам при каждой итерации
        if (start === null) {
          start = time;
        }

        var progress = time - start,
            // Время выполненное - время начальное
        r = toBlock < 0 ? Math.max(widthTop - progress / speed, widthTop + toBlock) : Math.min(widthTop + progress / speed, widthTop + toBlock); // От прокрученного мы вычитаем (сколько пролистал пользователь деленное на скорость которую мы задали)
        // В противном случае всё наоборот и мы листаем вверх

        document.documentElement.scrollTo(0, r);

        if (r != widthTop + toBlock) {
          // Если r = полному расстоянию, до необходимого эл-та, то прекращаем анимацию
          requestAnimationFrame(step);
        } else {
          location.hash = hash;
        }
...
var showMoreStyles = function showMoreStyles(trigger, wrapper) {
  ...
  btn.addEventListener('click', function () {
    Object(_services_requests__WEBPACK_IMPORTED_MODULE_3__["getResourses"])('assets/db.json').then(function (res) {
...
var sliders = function sliders(slides, dir, prev, next) {
  ...
  var activateAnim = function activateAnim() {
    if (dir === 'vertical') {
    ...
    while (1) {
      switch (_context2.prev = _context2.next) {
        ...
        case 2:
          ...
          throw new Error("Error! Could not fetch ".concat(url, ", status ").concat(res.status));
